  ...
  <div class="text-black dark:text-white bg-white dark:bg-base-100 container mx-auto p-4">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
      <div class="flex space-x-4">
        <router-link to="/role/add" class="btn btn-primary">Add Role</router-link>
          ...
          <tr v-for="data in paginatedData" :key="data.id">
            ...
            <td class="flex flex-row gap-2">
              <router-link :to="`/department/update/${data.id}`" class="btn btn-sm btn-primary mb-2"
                >Edit</router-link
              >
              <button @click="deleteRole(data.id)" class="btn btn-sm btn-danger">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-else class="text-center py-8 text-gray-500">
      {{ roleData.length ? 'No matching results found' : 'No data available' }}
    </div>
  </div>
</template>
...
const deleteRole = async (departmentId) => {
  ...
  try {
    ...
    // Remove from table
    roleData.value = roleData.value.filter((u) => u.id !== roleId)

    // Recalculate pagination automatically
    totalItems.value = roleData.value.length
