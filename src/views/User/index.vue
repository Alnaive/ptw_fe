<template>
  ...
  <div class="text-black dark:text-white bg-white dark:bg-base-100 container mx-auto p-4">
      ...
      <table class="table w-full text-black dark:text-white">
        ...
        <tbody>
          <tr v-for="data in paginatedData" :key="data.id">
            ...
            <td>{{ data.name }}</td>
            <td>{{ data.username }}</td>
            <td>{{ data.departmentData?.name }}</td>
            <td>{{ data.roleData?.name }}</td>
            <td>{{ data.status }}</td>
            <td class="flex flex-row gap-2">
              <router-link :to="`/user/update/${data.id}`" class="btn btn-sm btn-primary mb-2"
                >Edit</router-link
              >
              <button @click="deleteUser(data.id)" class="btn btn-sm btn-danger">Delete</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-else class="text-center py-8 text-gray-500">
      {{ userData.length ? 'No matching results found' : 'No data available' }}
    </div>
  </div>
</template>
...
const deleteUser = async (userId) => {
  ...
  try {
    ...
    // Remove from table
    userData.value = userData.value.filter((u) => u.id !== userId)

    // Recalculate pagination automatically
    totalItems.value = userData.value.length
